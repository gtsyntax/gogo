...
function OrderItem({ order }) {
  ...
  const orderAnswered = useSelector((state) => state.order.answered);
  return (
    <div className="shadow-lg rounded p-8 flex divide-x">
      ...
      <div className="py-4 px-8">
        ...
        <button className="bg-brand-primary my-2 py-2 px-8 text-brand-white rounded">
          Mark for Delivery
        </button>
      </div>
      ...
      <div className="py-4 px-8">
        ...
        {orderAnswered ? (
          orderStatus ? (
            <div className="accpeted">order accepted</div>
          ) : (
            <div className="declined">order declined</div>
          )
        ) : (
          <div className="still">still processing</div>
        )}
      </div>
    </div>
  );
